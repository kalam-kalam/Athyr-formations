...
gulp.task('task-admin-sass', function () {

    return gulp.src(path.resources.sass + '/admin.scss')
        .pipe(sass({
            onError: console.error.bind(console, 'SASS ERROR')
        }))
        .pipe(minify())
...
gulp.task('task-js', function () {
    ...
    return gulp.src(path.resources.js + '/front.js')
        .pipe(uglify())
        .pipe(rename({suffix: '.min'}))
        .pipe(gulp.dest(path.public.js))
        .pipe(browserSync.reload({
...
gulp.task('watch', ['browserSync', 'task-sass', 'task-js', 'task-admin-sass', 'task-blade'], function () {
    gulp.watch(path.sass, ['task-sass']);
    gulp.watch(path.js, ['task-js']);
    gulp.watch(path.blade, ['task-blade']);
    gulp.watch(path.sass, ['task-admin-sass'])
});
